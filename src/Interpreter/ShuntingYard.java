...
public class ShuntingYard {
    ...
    public Expression calc(String exp, CommandFactory commandFactory){
        ...
        for(String token : tokens)
        {
            ...
            if(token.matches("^[a-zA-Z0-9_]+$")){
                if(commandFactory.StringSymbolTable.containsKey(token))
                    queue.add(String.valueOf(commandFactory.PathTable.get(commandFactory.StringSymbolTable.get(token))));
                else if (commandFactory.DoubleSymbolTable.containsKey(token)){
                    queue.add(String.valueOf(commandFactory.DoubleSymbolTable.get(token)));
                }
                else
                    try{
        ...
        for(String str : queue) {
            if(isDouble(str))
                ...
                stackExp.push(new VarNumber(str, commandFactory));
            else {
